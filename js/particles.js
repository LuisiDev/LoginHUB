      ...
      if (orientationSupport && !desktop) {
        window.addEventListener(
          ...
          function () {
            ...
            tiltY = Math.min(Math.max(-event.beta, -10), 10);
            tiltX = Math.min(Math.max(-event.gamma, -10), 10);
          },
          true
        );
      }
    ...
    Particle.prototype.updatePosition = function () {
      if (options.parallax) {
        if (orientationSupport && !desktop) {
          ...
          var ratioX = (winW - 0) / (10 - -10);
          pointerX = (tiltX - -10) * ratioX + 0;
          // Map tiltY range [-30,30] to range [0,winH]
          var ratioY = (winH - 0) / (10 - -10);
          pointerY = (tiltY - -10) * ratioY + 0;
        } else {
          pointerX = mouseX;
          pointerY = mouseY;
        }
